...
- **`feed_pages.rb`** (since #6/#26) — generates all RSS feeds from two `_includes/` templates: one RSS2 feed per category page at `<category permalink>feed/` (redirect stubs derived from the category page's `redirect_from` list) plus the main RSS2 feed at `/feed/` and its `/feed.xml` alias. Replaced 25 hand-written XML files; `jekyll-feed` (Atom) is retired from the plugins list (gem still installed, not loaded). **Gotcha baked into the plugin:** Jekyll 4's LiquidRenderer caches parsed templates keyed by page *path*, so every generated `PageWithoutAFile` needs a unique dir/name or they all render the first page's template. Runs at `priority :high` so `jekyll-redirect-from` (`:normal`) sees the generated pages' `redirect_from` data.
...
`sitemap.xml` is generated by the `jekyll-sitemap` plugin (the hand-written source file was deleted in #26 — it was submitting noindexed redirect stubs and junk to Google). Pages opt out with `sitemap: false` frontmatter; XML feed pages whose URLs end in `/` count as `html_pages`, so `feed_pages.rb` sets that flag on everything it generates.
...
- Commit messages should explain WHY. During the cleanup phase, that "why" usually points at a GitHub issue (`Closes #N`). End commit messages with a `Co-Authored-By:` trailer crediting the Claude model that actually wrote them (e.g. `Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>`).
